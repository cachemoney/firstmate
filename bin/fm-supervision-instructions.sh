...
checkpoint_seconds_agy=${FM_AGY_WATCH_CHECKPOINT:-180}
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    agy)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision with a foreground checkpoint: bin/fm-watch-checkpoint.sh --seconds ' "$checkpoint_seconds_agy" '.'
      ;;
    pi|pi-signed)
