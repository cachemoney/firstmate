...
# non-zero exit, never a warning and never a silent skip. In secondmate-home
# mode, <home> must be a seeded secondmate home carrying its regular marker
# matching <id>, AGENTS.md, bin/, and internal subdirectories. Only the
# launching user's own store is written, it must be a regular file this uid owns,
# every unrelated key and entry is preserved, and the replacement is atomic.
set -u
unset CDPATH \
